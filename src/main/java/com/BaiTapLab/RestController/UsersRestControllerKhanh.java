...
import com.BaiTapLab.Service.MailerServiceKhanh;
import com.BaiTapLab.Service.UsersService;
import com.BaiTapLab.Service.UsersServiceKhanh;
...
public class UsersRestControllerKhanh {

	@Autowired
	MailerServiceKhanh mailerService;
	...
	@Autowired
	private UsersServiceKhanh usersService;
	...
	@Autowired
	RoleRepository vaitroRepository;
	...
	public List<Map<String, Object>> MapToData(List<Object[]> results) {
		...
		for (Object[] row : results) {
			...
			sanPham.put("hovaten", row[2]);
			sanPham.put("ten_vai_tro", row[1]);
			sanPham.put("hoat_dong", row[3]);
			sanPham.put("password", row[4]);
			sanPham.put("so_dien_thoai", row[5]);
...
//			
		...
		try {
			Users createdUser = usersService.createUserWithImageAndDetails(user, role, hinhAnh);
	...
	public ResponseEntity<Map<String, Object>> updateUserWithImageAndDetails(
		...
		// Attempt to save user details with service
		try {
			Users updatedUser = usersService.createUserWithImageAndDetailss(user, roles, hinhAnh);
